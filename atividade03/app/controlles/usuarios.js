...
module.exports.excluirUsuario = function(req, res){
    ...
    if(idUser == id_userLogado){
        let promise = Usuario.findOneAndDelete(id_userLogado)
        promise.then(
            function (usuario) {
            ...
            function (error) {
                res.status(400).json({ mensagem: "Não foi possível remover usuário, erro interno.", error: error })
            }
        )
    }else{
        res.status(401).json({ mensagem: "Não foi possível remover usuário. (token =!)"})
    }
}
